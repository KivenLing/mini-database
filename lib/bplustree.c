...
{
    struct bplus_leaf *first = first_leaf_node(tree);
    struct bplus_leaf *last = last_leaf_node(tree);
    if (first != NULL)
    {
        key_t min_k = first->key[0];
        key_t max_k = last->key[last->entries - 1];
        for (key_t i = min_k; i <= max_k; i++)
        {
...
struct bplus_leaf *first_leaf_node(struct bplus_tree *tree)
{
    if (!tree)
        return NULL;
    ...
    if (!root)
        return NULL;
    struct bplus_non_leaf *temp = NULL;
    while (!is_leaf(root))
    {
        temp = (struct bplus_non_leaf *)root;
        root = temp->sub_ptr[0];
    }
    return (struct bplus_leaf *)root;
}

struct bplus_leaf *last_leaf_node(struct bplus_tree *tree)
{
    if (!tree)
        return NULL;
    struct bplus_node *root = tree->root;
    ...
    {
        temp = (struct bplus_non_leaf *)root;
        root = temp->sub_ptr[temp->children - 1];
    }
    return (struct bplus_leaf *)root;
}

struct bplus_leaf *next_leaf_node(struct bplus_leaf *cur)
{
    return list_next_entry(cur, link);
}
